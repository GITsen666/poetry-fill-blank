...
function loadPoetry() {
  ...
  let text = poetry.text;
  for (let i = 0; i < poetry.blanks.length; i++) {
    text = text.replace(' ', `<input type="text" id="blank-${i}"/>`);
  }
...
function checkAnswers() {
  ...
  const poetry = poetryData[currentPoemIndex];
  for (let i = 0; i < poetry.blanks.length; i++) {
    if (!document.getElementById(`blank-${i}`).value) { ok = false; break; }
  }
  if (ok) {
    document.getElementById('feedback').textContent = "恭喜！你答对了";
    currentPoemIndex++;
    if (currentPoemIndex < poetryData.length) loadPoetry();
    else document.getElementById('feedback').textContent = "游戏结束！";
  } else {
